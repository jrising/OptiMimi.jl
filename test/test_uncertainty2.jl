...
for samplemult in [1, 2]
    for mcperlife in [5, 10, 50]
        ...
        tic()
        soln = solution(prob, () -> repmat([.25], 10), :biological, mcperlife, 10000*samplemult)
        time = toc()
        ...
        tic()
        soln = solution(prob, () -> repmat([.25], 10), :sampled, mcperlife, 100*samplemult)
        time = toc()
