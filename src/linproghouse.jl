...
* `namedictionary::Dict{Symbol, Symbol}`: The names used in `parameters` and `constraints` must be unique, but can refer to the same parameter or variable by including an entry in this disctionary mapping the unique name to the true variable name.
